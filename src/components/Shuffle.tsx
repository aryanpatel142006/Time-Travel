...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  const baseTw = 'inline-block whitespace-normal break-words will-change-transform uppercase text-4xl leading-none';
  const userHasFont = className && /font[-[]/i.test(className);

  const fallbackFont = userHasFont ? {} : { fontFamily: `'Press Start 2P', sans-serif` };

  const computedFontSize = fontSize !== undefined ? (typeof fontSize === 'number' ? `${fontSize}px` : fontSize) : undefined;
